...
const Register = () => {
  ...
  const handleRegister = async () => {
    try {
      ...
      const response = await axios.post(`${API}/authentication/register/peminjam`, formData, {
        signal,
      });

      console.log('resp register', response.data);
          ...
          <div className="rounded-md shadow-sm -space-y-px flex flex-col">
            <div>
              ...
              <input
                ...
                required
                className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 bg-gray-100 placeholder-gray-500 text-gray-900 rounded-t-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                placeholder="Name"
              />
            </div>
            <div>
