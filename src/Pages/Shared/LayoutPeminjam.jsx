...
const Sidebar = () => {
  ...
  useEffect(() => {
    if (location.pathname === '/dashboard/peminjam') {
      navigate('dashboard-peminjam', { state: location.state });
    }
  ...
  return (
    <div className="fixed top-0 left-0 h-full w-16.5 bg-blue-600 text-white flex flex-col items-start justify-start flex-wrap gap-3">
      ...
      <button
        onClick={() => navigate('/dashboard/peminjam/dashboard-peminjam')}
        className={`px-2 py-1 text-sm hover:bg-blue-700 w-full text-left ${
          actived === '/dashboard/peminjam/dashboard-peminjam' ? 'bg-blue-700' : ''
        }`}
        style={{ width: '130px' }}
      >
        Dashboard
      ...
      <button
        onClick={() => navigate('/dashboard/peminjam/buat-pinjaman')}
        className={`px-2 py-1 text-sm hover:bg-blue-700 w-full text-left ${
          actived === '/dashboard/peminjam/buat-pinjaman' ? 'bg-blue-700' : ''
        }`}
        style={{ width: '130px' }}
      >
        Buat Pinjaman
      ...
      <button
        onClick={() => navigate('/dashboard/peminjam/kembalikan-buku')}
        className={`px-2 py-1 text-sm hover:bg-blue-700 w-full text-left ${
          actived === '/dashboard/peminjam/kembalikan-buku' ? 'bg-blue-700' : ''
        }`}
        style={{ width: '130px' }}
      >
        Kembalikan Buku
